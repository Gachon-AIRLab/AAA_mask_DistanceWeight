...
def maskrcnn_loss(mask_logits, proposals, gt_masks, gt_labels, mask_matched_idxs):
    ...
    mask_targets = mask_targets / 255  #for WD

    
    # logit = mask_logits[len(mask_logits),-1, 28, 28]
    
    # plt.imshow(tf.ToPILImage()(logit.cpu()))
    # plt.show()
    ...
    # n => roi
    # print(mask_logits.shape) #=> n * 2 * 28 * 28 (background, mask)
    # print(mask_targets.shape) #=> n * 28 * 28
    ...
    #mask_logits = F.softmax(mask_logits, dim = 1)
    #mask_logits = F.normalize(mask_logits, dim = 1)
#    mask_logits = F.log_softmax(mask_logits, dim = 1)
    #mask_logits = F.sigmoid(mask_logits)


    tensor1 = mask_logits[:,0,:,:]                                                                                                                          
    tensor2 = mask_logits[:,1,:,:]

    # print(mask_logits.shape) #=> n * 28 * 28
    # exit(0)
    # mask_logits = mask_logits / mask_logits.sum(0).expand_as(mask_logits)
    # print(tensor2[0])
    ...
    mask_loss = F.mse_loss(
        tensor2, mask_targets
    )

    #======show image======
    # fig = plt.figure()
    # ax1 = fig.add_subplot(1, 2, 1)
    # ax1.imshow(tf.ToPILImage()(mask_targets[0].cpu()))
    
    # ax2 = fig.add_subplot(1, 2, 2)
    # ax2.imshow(tf.ToPILImage()(tensor2[0].cpu()))
    # plt.draw()
    ...
    def forward(self, features, proposals, image_shapes, targets=None):
        ...
        if self.has_mask:
            ...
            if self.training:
                ...
                # plt.imshow(tf.ToPILImage()(gt_masks[0].cpu()))
                # plt.show()
                # exit(0)
